...
const { createStorageLayout, resolveBaseDir } = require('./storage.js');
...
function derivePreview(messages) {
  ...
  if (!last) return null;
  return (last.content.trim().split('\n')[0] ?? '').slice(0, 160);
}
...
async function loadConversation(sessionId, baseDir) {
...
async function listConversations(baseDir) {
  const directories = await ensureDirectories(baseDir);
  let entries = [];
  try {
...
async function getConversation(sessionId, baseDir) {
...
async function createConversation({ baseDir, sessionId, firstMessage }) {
  const directories = await ensureDirectories(baseDir);
...
async function saveConversation(conversation, directories, filePath) {
